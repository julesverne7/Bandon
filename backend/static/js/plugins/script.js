...
document.addEventListener('DOMContentLoaded', function () {
  // feather icon start
  // feather.replace(); // Uncomment this line if feather icons are used in the project
...
function setLayout() {
  ...
  if (layout !== null && layout !== '') {
    ...
    if (layout === 'horizontal') {
      ...
      script.src = layout_horizontal_js_loc; // Load script for horizontal layout
...
// Prevent submenu toggling / sidebar auto-close when clicking actual navigation
// links inside submenus. We only stop propagation for anchors inside
// `.pc-submenu` so that top-level menu links (which toggle submenus) still
// receive their click events.
var _submenu_links = document.querySelectorAll('.pc-sidebar .pc-navbar .pc-submenu a');
for (var _i = 0; _i < _submenu_links.length; _i++) {
  _submenu_links[_i].addEventListener('click', function (e) {
    // Stop the event from bubbling up to parent li click handlers which
    // toggle/collapse the submenu. This prevents the drawer from closing or
    // toggling while the browser navigates to the new page.
    e.stopPropagation();
    // Do not call rm_menu() here; let the browser navigate. If mobile
    // auto-close is required, handle it separately (see notes).
  });
}
